...
import waves.token.walkmiles.models.DataSteps;
import waves.token.walkmiles.models.StepsCount;
...
public class AdapterData1 extends RecyclerView.Adapter<AdapterData1.CustomRecyclerView>{
    ...
    public CustomRecyclerView onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            case TYPE_HEADER: {
                layoutView = LayoutInflater.from(parent.getContext()).inflate(waves.token.walkmiles.R.layout.list_item_card_big, parent, false);
                CustomRecyclerView rcv = new CustomRecyclerView(layoutView);
                return rcv;
            }
            case TYPE_CELL: {
                layoutView = LayoutInflater.from(parent.getContext()).inflate(waves.token.walkmiles.R.layout.list_item_card_small, parent, false);
                CustomRecyclerView rcv = new CustomRecyclerView(layoutView);
                return rcv;
            }
        }
    ...
    public class CustomRecyclerView extends RecyclerView.ViewHolder {
        ...
        CustomRecyclerView(View itemView) {
            super(itemView);
            txtDate = itemView.findViewById(waves.token.walkmiles.R.id.txt_date);
            txtResumen = itemView.findViewById(waves.token.walkmiles.R.id.txt_resumen);
            txtStep = itemView.findViewById(waves.token.walkmiles.R.id.txt_step);
            txtKm = itemView.findViewById(waves.token.walkmiles.R.id.txt_km);
            txtWm = itemView.findViewById(waves.token.walkmiles.R.id.txt_wm);
            chart = itemView.findViewById(waves.token.walkmiles.R.id.chart);
        }
    }
}